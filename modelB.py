...
def modelB_classifier(train_language, test_language, task):


	dual_training_data  = create_train_multilingual_dual_data_4sub(train_language, test_language)

	model = train_bilstm_attention_jointalignment_sub_pseudo(test_language, dual_training_data, Tx, Ty, epochs=epochs)
	...
	for test_set in [train_language, test_language]:
		...
		acc_o, f1_o = evaluate_bilstm_attention(model, X_test, Y_test)
...
modelB_classifier(sys.argv[1], sys.argv[2], 'request')
